 ...
 */
add_action( 'admin_enqueue_scripts', 'kortez_lawyer_widgets_backend_enqueue' );
function kortez_lawyer_widgets_backend_enqueue()
{
    wp_enqueue_style( 'kortez-lawyer-getting-started', get_template_directory_uri() . '/inc/getting-started/getting-started.css' );
    wp_register_script( 'kortez-lawyer-custom-widgets', get_template_directory_uri() . '/inc/getting-started/getting-started.js', array( 'jquery' ), true );
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'kortez-lawyer' ),
        'nonce'    => wp_create_nonce( 'kortez_lawyer_demo_import_nonce' ),
        'noncen'    => 'kortez_lawyer_demo_import_nonce',
        'adminurl'    => esc_url( admin_url() ),
    );
    wp_localize_script( 'kortez-lawyer-custom-widgets', 'kortez_lawyer_adi_install', $translation );
    wp_enqueue_media();
    wp_enqueue_script( 'kortez-lawyer-custom-widgets' );
}

/**
 * Class to handle notices and Advanced Demo Import
 *
 * Class kortez_lawyer_Notice_Handler
 */
class kortez_lawyer_Notice_Handler {
	...
	public function __construct() {
        ...
        add_action( 'admin_notices', array( $this, 'kortez_lawyer_theme_info_welcome_admin_notice' ), 3 );
        add_action( 'wp_ajax_kortez_lawyer_getting_started', array( $this, 'kortez_lawyer_getting_started' ) );
    }

    /**
     * Display an admin notice linking to the about page
     */
    public function kortez_lawyer_theme_info_welcome_admin_notice() {
        $theme = wp_get_theme();
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            ...
            if ( is_plugin_active( 'advanced-import/advanced-import.php' ) && is_plugin_active( 'kortez-toolset/kortez-toolset.php' ) && is_plugin_active( 'elementor/elementor.php' ) && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'elementskit-lite/elementskit-lite.php' ) ){
                echo ('<div class="fs-notice success"><p>'.esc_html__( 'All plugins are installed. To hide this notice click on dismiss.', 'kortez-lawyer' ).'</p></div>');
            }else{
                /* translators: %s - Theme name*/
                echo ( '<p><strong>' . sprintf( esc_html__( 'Welcome to %1$s.', 'kortez-lawyer' ), esc_attr( $theme->get( 'Name' ) ) ) . '</strong></p><p class="plugin-notice">'.esc_html__( 'Clicking on get started will install and activate Kortez Toolset, Advanced Import, Elementor, ElementsKit Lite and Contact Form 7.', 'kortez-lawyer' ).'</p><p><a href="#" class="kortez-lawyer-install-plugins button button-primary">' . sprintf( esc_html__( 'Get started with %s','kortez-lawyer' ), esc_attr( $theme->get( 'Name' ) ) ) . '</a></p>' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
	 ...
	 * return void
	 */
	public function kortez_lawyer_getting_started() {

		check_ajax_referer( 'kortez_lawyer_demo_import_nonce', 'security' );
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'kortez-lawyer' );
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'kortez-lawyer' );
...
new kortez_lawyer_Notice_Handler;